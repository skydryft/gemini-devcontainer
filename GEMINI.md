...
- Test: `bundle exec rspec` (use RSpec, not Minitest)
...
### Session notes
Write to `.gemini/session-notes/YYYY-MM-DD-topic.md` at milestones and before
ending. On startup, check for prior notes before asking the user to re-explain.
</workflow>
...
- Use Rails conventions: RESTful routes, ActiveRecord, MVC, partials for views.
- Use RSpec for all tests. Never generate or write Minitest. Place specs in `spec/`.
</constraints>
